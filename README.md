...
<p><b>Details</b><p>
  OCI Python Fn function sample to process an AuditEvent triggered by the OCI Events service
  Audit events emitted by the audit service are wire through the Events service which trigger an Fn fucntion to analyse the Audit event. 
  The sample looks for AuditEvents with evntName = "CreateSubnet"  and checks if the subnet was created with a free form tag named "owner". If not it publishes a message on the OCI Notifiction topic.
...
<p>Update your local env with the following <a href="/https://github.com/fnproject/tutorials/blob/oci-py-example/python/cloud/oracle/oci-instances-func/setup_local.sh">script</a>.</p>
